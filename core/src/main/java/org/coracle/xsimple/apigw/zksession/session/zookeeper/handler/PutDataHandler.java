...
public class PutDataHandler extends GetDataHandler {
    ...
    public <T> T handle() throws Exception {
        if (zookeeper != null) {
            String path = ZookeeperHandler.SESSION_GROUP_PATH + ZookeeperHandler.NODE_SEP + id;
            // 检查指定的Session节点是否存在
            Stat stat = zookeeper.exists(path, false);
            //如果节点存在则删除之
            if (stat != null) {
                //查找数据节点是否存在，不存在就创建一个
                String dataPath = path + ZookeeperHandler.NODE_SEP + key;
                stat = zookeeper.exists(dataPath, false);
                if (stat == null) {
                    //创建数据节点
                    zookeeper.create(dataPath, null, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
                    if (AbstractZookeeperHandler.LOGGER.isInfoEnabled()) {
                        AbstractZookeeperHandler.LOGGER.info("创建数据节点完成[" + dataPath + "]");
                    }
                }
                //在节点上设置数据，所有数据必须可序列化
                if (data instanceof Serializable) {
                    ...
                    if (AbstractZookeeperHandler.LOGGER.isInfoEnabled()) {
                        AbstractZookeeperHandler.LOGGER.info("更新数据节点数据完成[" + dataPath + "][" + data + "]");
                    }
